...
orgs_to_add = dbutils.entry_update("organismInfo", dborg_keys, orgs_unique)
...
expt_to_add = dbutils.entry_update("exptMetadata", dbexpt_keys, expt_unique)
...
samp_to_add = dbutils.entry_update("sampleID", dbsamp_keys, samp_unique)
...
gene_to_add = dbutils.entry_update("geneticInfo", dbgenetic_keys, gene_unique)
...
link_to_add = dbutils.entry_update("linkIDs", link_keys, link_unique)
...
exptcond_to_add = entry_update("sampleCondition",
                               ["sample_id", "condition_id"],
                               exptcond_unique)
if len(exptcond_to_add) > 0:
    dbconnect.engine.execute(sampleCondition.insert(), exptcond_to_add)
...
accum_to_add = dbutils.entry_update("sampleAccum",
                                    dbaccum_full_keys,
                                    accum_unique)
...
nftab = dbutils.add_version_info(paper_id, dirpath, "nascent", dbnf_keys)
for entry in nftab:
    for key in entry:
        entry[key] = str(entry[key])
bidirtab = dbutils.add_version_info(paper_id, dirpath, "bidir", dbbf_keys)
...
nf_to_add = dbutils.entry_update("nascentflowMetadata", dbnf_keys, nf_unique)
if len(nf_to_add) > 0:
...
bf_to_add = dbutils.entry_update("bidirflowMetadata", dbbf_keys, bidir_unique)
if len(bf_to_add) > 0:
    # Store null values correctly
...
exptnf_to_add = dbutils.entry_update("exptNascentflow",
